...
export const Attorneys = forwardRef((props, ref) => {
  ...
  return (
   ...
   <section ref={ref} id="attorneys" className={props.normal? `${AttorneysStyles.attorneys}  ${AttorneysStyles.normal}`: AttorneysStyles.attorneys}>
    
    <BackgroundImage styles={AttorneysStyles} url={Gavel}/>
    <Quote styles={props.normal? QuoteStyles: rest} quote='We are thorough and dedicated to all our clients.' author='Gerald Shwarts'/>
    <div className={AgentCardStyles.container}>
    <AgentCard styles={AgentCardStyles} title='Managing Attorney' image={Gerald} name='Gerald Swartz' position=""/>
    <AgentCard styles={AgentCardStyles} title='Senior Associate' image={Donna} name='Donna Lee' position='center'/>
    
    
    </div>
    <CallToAction styles={AttorneyCallToActionStyles} words={['605-908-5405']} buttons={['Free Consultation >']} icons={[<FaHeadset/>]} buttonPosition='bottom' />
   </section>
  )
}
)
